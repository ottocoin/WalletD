...
uint256 hashGenesisBlock("0xc055d2df5347e4b0889c315b7f84f5c76a2e94da90e5f654a2aaae7949cfa0c8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OTTOCOIN: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1491588117; // Line: 2815
...
const string strMessageMagic = "OTTOCOIN Signed Message:\n";
...
{
    // OTTOCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, unsigned int nTime, int64 nFees)
{
    int64 nSubsidy = 0 * COIN;
    if (nHeight == 5)
    {
        nSubsidy = 9999999 * COIN; //Premine
    }
	else if(nTime >= POW_REAL_START_TIME){
        nSubsidy = 10 * COIN;
    }
...
static const int64 nTargetTimespan = 5 * 60; // OTTOCOIN: 5 min
static const int64 nTargetSpacing = 2 * 60; // OTTOCOIN: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // OTTOCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 2; 
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2) { DiffMode = 2; } // OTTOCOIN, 2 blocks after first legacy diff adjustment
        }
        else {         
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, pindex->nTime, nFees))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nTime, nFees)));
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // OTTOCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // OTTOCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "OTTOCOIN launch 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 1602268;
        block.nTime = 1491588117;
        
        if (fTestNet)
        {

            block.nNonce = 115;
            block.nTime = 1491577117;
        }
      ...
      block.print();
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0xe4531e4dd30ba761b8c015c6935535b0dba9bb6e3884ce68ff29d13f6d276af7"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xef, 0xba, 0x2e }; 
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, pblock->nTime, nFees);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OTTOCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OTTOCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OTTOCOINMiner(CWallet *pwallet)
{
    printf("OTTOCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("OTTOCOIN-miner");
    ...
    try { while (true) {
        ...
        printf("Running OTTOCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OTTOCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OTTOCOINMiner, pwallet));
}
